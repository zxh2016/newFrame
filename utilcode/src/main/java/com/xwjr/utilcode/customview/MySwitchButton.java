 ...
 * 仿ios实现开关效果
 *
 <attr name="msb_shadow_radius" format="reference|dimension"/>       阴影半径
 <attr name="msb_shadow_offset" format="reference|dimension"/>       阴影偏移
 <attr name="msb_shadow_color" format="reference|color"/>            阴影颜色
 <attr name="msb_uncheck_color" format="reference|color"/>           关闭颜色
 <attr name="msb_checked_color" format="reference|color"/>           开启颜色
 <attr name="msb_border_width" format="reference|dimension"/>        边框宽度
 <attr name="msb_checkline_color" format="reference|color"/>         开启指示器颜色
 <attr name="msb_checkline_width" format="reference|dimension"/>     开启指示器线宽
 <attr name="msb_uncheckcircle_color" format="reference|color"/>     关闭指示器颜色
 <attr name="msb_uncheckcircle_width" format="reference|dimension"/> 关闭指示器线宽
 <attr name="msb_uncheckcircle_radius" format="reference|dimension"/>关闭指示器半径
 <attr name="msb_checked" format="reference|boolean"/>               是否选中
 <attr name="msb_shadow_effect" format="reference|boolean"/>         是否启用阴影
 <attr name="msb_effect_duration" format="reference|integer"/>       动画时间，默认300ms
 <attr name="msb_button_color" format="reference|color"/>            按钮颜色
 <attr name="msb_show_indicator" format="reference|boolean"/>        是否显示指示器，默认true：显示
 <attr name="msb_background" format="reference|color"/>              背景色，默认白色
 <attr name="msb_enable_effect" format="reference|boolean"/>         是否启用特效，默认true.
 */
...
public class MySwitchButton extends View implements Checkable {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        shadowRadius = optPixelSize(typedArray,
                R.styleable.MySwitchButton_msb_shadow_radius,
                dp2pxInt(2.5f));//dp2pxInt(2.5f);

        shadowOffset = optPixelSize(typedArray,
                R.styleable.MySwitchButton_msb_shadow_offset,
                dp2pxInt(1.5f));//dp2pxInt(1.5f);

        shadowColor = optColor(typedArray,
                R.styleable.MySwitchButton_msb_shadow_color,
                0X33000000);//0X33000000;

        uncheckColor = optColor(typedArray,
                R.styleable.MySwitchButton_msb_uncheck_color,
                0XffDDDDDD);//0XffDDDDDD;

        checkedColor = optColor(typedArray,
                R.styleable.MySwitchButton_msb_checked_color,
                0Xff51d367);//0Xff51d367;

        borderWidth = optPixelSize(typedArray,
                R.styleable.MySwitchButton_msb_border_width,
                dp2pxInt(1));//dp2pxInt(1);

        checkLineColor = optColor(typedArray,
                R.styleable.MySwitchButton_msb_checkline_color,
                Color.WHITE);//Color.WHITE;

        checkLineWidth = optPixelSize(typedArray,
                R.styleable.MySwitchButton_msb_checkline_width,
                dp2pxInt(1f));//dp2pxInt(1.0f);

        checkLineLength = dp2px(6);

        int buttonColor = optColor(typedArray,
                R.styleable.MySwitchButton_msb_button_color,
                Color.WHITE);//Color.WHITE;

        int effectDuration = optInt(typedArray,
                R.styleable.MySwitchButton_msb_effect_duration,
                300);//300;

        isChecked = optBoolean(typedArray,
                R.styleable.MySwitchButton_msb_checked,
                false);

        showIndicator = optBoolean(typedArray,
                R.styleable.MySwitchButton_msb_show_indicator,
                true);

        background = optColor(typedArray,
                R.styleable.MySwitchButton_msb_background,
                Color.WHITE);//Color.WHITE;

        enableEffect = optBoolean(typedArray,
                R.styleable.MySwitchButton_msb_enable_effect,
                true);
